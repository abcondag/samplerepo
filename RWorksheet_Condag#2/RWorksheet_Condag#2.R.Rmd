...
x
```
a & b. Print the results. Then access x[c("first", "third")].
```{r}
x
names(x)
x[c("first", "third")]
```
First it outputs the elements of x. Then names(x) access and outputs only the names or string in the element. Lastly x[c("first", "third")] outputs the elements corresponding to first and third.
x[c("first", "third")]
...
x
```
a & b. Modify 2nd element and change it to 0.
```{r}
x[2] <- 0
x
```
It changes the 2nd element from -2 into 0
...
mrCruzdata
```
b. b. What is the average fuel expenditure of Mr. Cruz from Jan to June? Note: Use ‘weighted.mean(liter, purchase).
```{r}
